...
static void  USBD_CMPSIT_HIDDesc(USBD_HandleTypeDef *pdev, uint32_t pConf, __IO uint32_t *Sze, uint8_t speed);
...
/**
  ...
  * @retval status
  */
//-- 구성 디스크립터 누적 크기를 되감는다.
//
//   CurrFSConfDescSz 는 파일 스코프 정적이라 USBD_Init() 이 건드리지 못한다.
//   되감지 않고 usbBegin() 을 다시 부르면 같은 인터페이스가 뒤에 또 붙어
//   구성 디스크립터가 두 배로 커진다. 부트로더 점프 전 usbDeInit() 후에
//   재시작하는 경로가 생길 때를 대비한다.
//
void USBD_CMPSIT_Reset(void)
{
  CurrFSConfDescSz = 0U;
#ifdef USE_USB_HS
  CurrHSConfDescSz = 0U;
...
uint8_t  USBD_CMPSIT_AddToConfDesc(USBD_HandleTypeDef *pdev)
{
  ...
  switch (pdev->tclasslist[pdev->classId].ClassType)
  {
    ...
    case CLASS_TYPE_HID:
      /* HID 는 FS/HS 모두 같은 패킷 크기를 쓴다. */
      pdev->tclasslist[pdev->classId].CurrPcktSze = HID_EP_SIZE;

      /* 인터페이스 1개(vendor-defined). IAD 는 필요 없다. */
      idxIf = USBD_CMPSIT_FindFreeIFNbr(pdev);
      pdev->tclasslist[pdev->classId].NumIf = 1U;
      pdev->tclasslist[pdev->classId].Ifs[0] = idxIf;

      /* 엔드포인트 2개. IN/OUT 둘 다 인터럽트 타입이라 방향으로만 구분된다.
         usbd_hid.c 가 USBD_CoreGetEPAdd() 로 방향을 지정해 되찾아간다. */
      ...
      USBD_CMPSIT_HIDDesc(pdev, (uint32_t)pCmpstFSConfDesc, &CurrFSConfDescSz, (uint8_t)USBD_SPEED_FULL);

#ifdef USE_USB_HS
      USBD_CMPSIT_HIDDesc(pdev, (uint32_t)pCmpstHSConfDesc, &CurrHSConfDescSz, (uint8_t)USBD_SPEED_HIGH);
#endif /* USE_USB_HS */
...
#if USBD_CMPSIT_ACTIVATE_HID == 1
/**
  * @brief  USBD_CMPSIT_HIDDesc
  *         vendor-defined HID 인터페이스 하나를 구성 디스크립터에 덧붙인다.
  *
  *         키보드/마우스가 아니라 64바이트 IN/OUT 리포트를 주고받는 통신 채널이다.
  *         bInterfaceSubClass/Protocol 을 0 으로 두어야 OS 가 부트 장치로 오인하지
  *         않고, WebHID 도 usagePage 0xFF00 으로 골라낼 수 있다.
  * @param  pdev: device instance
  * @param  pConf: Configuration descriptor pointer
  * @param  Sze: pointer to the current configuration descriptor size
  * @retval None
  */
static void  USBD_CMPSIT_HIDDesc(USBD_HandleTypeDef *pdev, uint32_t pConf,
                                 __IO uint32_t *Sze, uint8_t speed)
{
  ...
  __USBD_CMPSIT_SET_IF(pdev->tclasslist[pdev->classId].Ifs[0],
                       ...
                       0x00U,  /* bInterfaceSubClass: 0 = no boot */
  ...
  pHidDesc->wItemLength         = HID_REPORT_DESC_SIZE;
  *Sze                         += (uint32_t)sizeof(USBD_HIDDescTypeDef);

  /* Append Endpoint descriptors to Configuration descriptor */
  __USBD_CMPSIT_SET_EP(pdev->tclasslist[pdev->classId].Eps[0].add, USBD_EP_TYPE_INTR,
                       HID_EP_SIZE, HID_HS_BINTERVAL, HID_FS_BINTERVAL);

  __USBD_CMPSIT_SET_EP(pdev->tclasslist[pdev->classId].Eps[1].add, USBD_EP_TYPE_INTR,
                       HID_EP_SIZE, HID_HS_BINTERVAL, HID_FS_BINTERVAL);
