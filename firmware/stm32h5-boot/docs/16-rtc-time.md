...
## 부트 이벤트 로그가 안 남는 것처럼 보일 때

같은 펌웨어를 다시 올리면 기록이 남지 않는다. 슬롯 내용이 FIRM 과 같으면
`bootGetPendingSlot()` 이 -1 이고, 부트로더는 **적용할 것이 없어서 아무 일도
하지 않는다.** 재부팅만 하고 끝난다.

그런데 호스트에는 "적용 요청 완료" 라고 나갔다. 실패도 아닌데 아무 일도 일어나지
않았고, 로그에도 안 남으니 "기록이 왜 안 남지?" 가 된다. 조용히 아무 일도 하지
않는 것이 조용히 실패하는 것만큼 나쁘다.

앱이 `ERR_BOOT_NO_PENDING (0x0016)` 으로 알려주고 **리셋하지 않는다.**
호스트(웹/툴)는 이것만 따로 안내한다.

```
  적용   할 것이 없다. 슬롯 이미지가 지금 실행 중인 것과 같다.
```

부트로더 모드에서는 그대로 적용한다. 이미 부트로더에 들어와 있으니 사용자는
앱으로 넘어가기를 기대하고, 같은 내용을 다시 복사하는 것은 무해하다.
...
**표는 재연결 때 비운다.** 이전 연결의 목록이 남아 있으면 업데이트 뒤 다시
연결했을 때 옛 목록을 보고 오해한다. 보드 플래시의 로그는 건드리지 않는다 -
전원을 뽑아도 남는 것이 그 로그의 존재 이유다. 지우려면 `boot log clear` 다.
