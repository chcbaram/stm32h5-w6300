...
| 19 | **보드 자체 웹서버** — HTTP + POST /cmd | ✅ curl·JXA 검증 (브라우저 확인 남음) |

**빌드**: 부트로더 85,848 B / 126 KB (66.5%) · 앱 191,872 B / 445 KB (42.1%)
...
1. **NVS 가상 EEPROM** (`14-roadmap.md` D)
2. **부트 타임아웃** — 부트 모드에서 일정 시간 뒤 앱 실행. 조건(호스트 미연결일
   때만)까지 정리했으나 사용자가 보류함
3. mDNS 응답기 — `<이름>.local` 로 IP 없이 접속. 브라우저에서 **목록을 얻는 것은
   여전히 불가능**하다(JS 에 mDNS API 가 없다). IP 를 외우지 않아도 되는 것이 이득
...
29. **macOS 의 "디스크를 제대로 꺼내지 않았습니다" 는 못 없앤다.** `TEST UNIT READY`
