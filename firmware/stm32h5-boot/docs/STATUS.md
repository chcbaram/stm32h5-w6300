...
**빌드**: 부트로더 85,784 B / 126 KB (66.5%) · 앱 166,728 B / 445 KB (36.6%)
...
1. **네트워크 패널** — 앱이 자기 IP/MAC 을 알려주는 전용 커맨드를 만들고,
   LAN 을 스캔해 보드 목록을 돌려준다 (`14-roadmap.md` B).
   지금은 CLI 패널에서 `wiznet info` 로만 볼 수 있다.
...
21. **macOS 의 "디스크를 제대로 꺼내지 않았습니다" 는 못 없앤다.** `TEST UNIT READY`
