...
1. **mDNS 응답기** — `stm32h5-d2fc.local` 로 IP 없이 접속. 이름은 MAC 에서 만들어
   충돌을 원천 차단한다(`20-unique-mac.md`). **소켓이 없다** - 0~7 을 telnet/DHCP/
   SNTP/cmd TCP/discovery/HTTP×3 이 다 쓴다. cmd TCP(3)를 HTTP POST 로 대체해
   비우는 것이 깨끗하다. 기능은 그대로고 네트워크 커맨드 경로가 하나로 준다.
2. **타깃 테스트 보강** — TCP OTA·HTTP·스캔·RTC 를 덮는 `test_06_net.py`.
   지금 회귀를 잡아주는 것이 호스트 유닛 50개뿐이고 그건 부트로더 슬롯 로직만 본다.
3. **NVS 가상 EEPROM** (`14-roadmap.md` D). 보드 이름·고정 IP 를 여기 둔다
4. **부트 타임아웃** — 조건(FIRM 유효 + 호스트 미연결)까지 정리했으나 보류 중
